...
class rss {
	...
	public function init() {
		...
		if(empty($this->rssid)) {
			...
			include template('content', 'rss',$default_style);
	...
	public function rsslist(){
		...
		//include template('content', 'rsslist');
		$sitelist  = getcache('sitelist','commons');
			$default_style = $sitelist[1]['default_style'];
				if (isMobile()){  
					$default_style=$default_style."_wap";  
			...
			include template('content', 'rsslist',$default_style);
	}
	...
	public function add(){
		...
		$db = pc_base::load_model('rss_model'); 
		$arr = $db->insert($data);
	...
	public function dianzan(){
		...
		$data['wid']=$_POST['wid'];
		$db = pc_base::load_model('dianzan_model'); 
		$arr = $db->insert($data);
		

		$dz = $db->get_all(array('userid'=>$_POST['userid']));
		$num = count($dz);
	...
	public function addcare(){
		...
		$db = pc_base::load_model('careinfo_model'); 
		$arr = $db->insert($data);
