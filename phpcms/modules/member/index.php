...
class index extends foreground {
	...
	public function init() {
		...
		$default_style = $sitelist[1]['default_style'];
			...
			if (isMobile()){  
				$default_style=$default_style."_wap";  
	...
	public function register() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$userinfo['mobile'] = (isset($_POST['mobile']) && is_username($_POST['mobile'])) ? $_POST['mobile'] : '';
			...
			if(!empty($_GET['verify'])) {
				...
				include template('member', 'register',$default_style);
	...
	public function account_manage_avatar() {
		...
		include template('member', 'account_manage_avatar',$default_style);
	}
	...
	public function account_manage_info() {
		if(isset($_POST['dosubmit'])) {
			...
			if (isMobile()){ 
					showM('修改成功','index.php?m=member&c=index&a=account_manage_info&t=1');
			}else{
			...
			include template('member', 'account_manage_info',$default_style);
	...
	public function account_manage_password() {
		if(isset($_POST['dosubmit'])) {
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				if ($res < 0) 
					if (isMobile()){
						showM($message_error[$res],HTTP_REFERER);
					}else{
						showmessage($message_error[$res]);
					}
			}
			if (isMobile()){
					showM('修改成功',HTTP_REFERER);
			}else{
					showmessage(L('operation_success'), HTTP_REFERER);
				}
		} else {
			...
			include template('member', 'account_manage_password',$default_style);
	...
	public function account_change_mobile() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			include template('member', 'account_change_mobile',$default_style);
		}
	}
	...
	public function public_get_password_type() {
		...
		$sitelist  = getcache('sitelist','commons');
			...
			include template('member', 'get_password_type',$default_style);
	}
	...
	public function account_manage_upgrade() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			include template('member', 'account_manage_upgrade',$default_style);
		}
	}
		...
		if(isset($_POST['dosubmit'])) {
			if(empty($_SESSION['connectid'])) {
				...
				if ($_SESSION['code'] != strtolower($code)) {
					$_SESSION['code'] = '';
					if (isMobile()){
						showM('验证码错误','index.php?m=member&c=index&a=login');
					}else{
			...
			if($r['islock']) {
				if (isMobile()){
					showM('用户被锁定','index.php?m=member&c=index&a=login');
				}else{
			...
			include template('member', 'login',$default_style);
		}
	}
	...
	public function favorite() {
		...
		if(isset($_GET['id']) && trim($_GET['id'])) {
			...
			include template('member', 'favorite_list',$default_style);
		}
	}
	 ...
	 */
	 public function commentlist(){
		...
		$sitelist  = getcache('sitelist','commons');
			...
			include template('member', 'comment_list',$default_style);
	 }
	 ...
	 */
	 public function tousu(){
		...
		$sitelist  = getcache('sitelist','commons');
			...
			include template('member', 'tousu',$default_style);
	 }
	...
	public function mini() {
		...
		$sitelist  = getcache('sitelist','commons');
			...
			include template('member', 'mini',$default_style);
	}
	...
	public function public_forget_password () {
		...
		if(isset($_POST['dosubmit'])) {
			...
			include template('member', 'forget_password',$default_style);
		}
	}
	...
	*/
	...
	public function public_forget_password_mobile () {
		...
		//处理提交申请，以手机号为准
			...
			include template('member', 'forget_password_mobile',$default_style);
		}
	}
	//通过用户名找回密码
	public function public_forget_password_username() {
		...
		//处理提交申请，以手机号为准
			...
			include template('member', 'forget_password_username',$default_style);
		}
	}
	...
	public function show_ziliao(){
		...
		$avatar = $this->client->ps_getavatar($this->memberinfo['phpssouid']);
			...
			include template('member', 'show_ziliao',$default_style);
	}
	
	//上传头像
	public function upload_avatar(){
		//phpsso_server/uploadfile/avatar/1/1/10/90x90.jpg  需要上传到的路径
		$avatar = $_FILES['face'];
		...
		if($avatar){
			...
			$fileurl = 'phpsso_server/uploadfile/avatar/'.$dir1.'/'.$dir2.'/'.$uid.'/';
			//echo $fileurl;
			
			if($avatar['size']>0){
				
				move_uploaded_file($avatar["tmp_name"],$fileurl."90x90.jpg");
	...
	public function publish_add(){
		...
		$title = $_POST['title'];
		$catid = $_POST['catid'];
		$copyfrom = $_POST['copyfrom'];//_data表
		$content = $_POST['neirong'];//_data表
		$posid = $_POST['posid'];
		//uploadfile/2017/0829/1.jpg
		$thumb = $_FILES['face'];
		
		$y=date("Y",time());  
		$md=date("md",time());
			
		$fileurl = 'uploadfile/'.$y.'/'.$md.'/';
		...
		if($thumb){
			...
			if($thumb['size']>0){
				...
				$filename = $y.$md.time();
				move_uploaded_file($thumb["tmp_name"],$fileurl.$filename.".jpg");
		...
		$info['catid'] = $catid;
		$info['posids'] = $posid;
 		$info['thumb'] = APP_PATH.$fileurl.$filename.".jpg";
		$info['status'] = 99;
		$info['description'] = str_cut($content,200);
		...
		if($catid==17||$catid==18||$catid==19){
			...
			if($yinpin['size']>0){
				...
				$filename = $y.$md.time();
				move_uploaded_file($yinpin["tmp_name"],$fileurl.$yinpin["name"]);
				$info['yinpin'] ="1";
				$info['yinpin_fileurl'] = APP_PATH.$fileurl.$yinpin["name"];
				$info['yinpin_filename'] ="1";
			}
			if($shipin['size']>0){
				if(!file_exists($fileurl)){  
				  mkdir($fileurl,0777);   
				...
				$filename = $y.$md.time();
				move_uploaded_file($shipin["tmp_name"],$fileurl.$shipin["name"]);
				
				$info['shipin'] ="1";
				$info['shipin_fileurl'] = APP_PATH.$fileurl.$shipin["name"];
				$info['shipin_filename'] ="1";
				
			}
			
			
			//音视频的保存地址 {"0":{"fileurl":"http:\/\/139.129.216.13\/twzb\/uploadfile\/2017\/0908\/20170908053352617.mp3","filename":"1"}}
			
			$this->content_db->set_model(15);
			
		}elseif($catid==20||$catid==21||$catid==22){
			$info['sp'] = $_FILES['shipin'];
			if($info['sp']['size']>0){
				if(!file_exists($fileurl)){  
				  mkdir($fileurl,0777);   
				}
				$filename = $y.$md.time();
				move_uploaded_file($info['sp']["tmp_name"],$fileurl.$info['sp']["name"]);
				
			}
			$info['sp_fileurl'] = APP_PATH.$fileurl.$info['sp']["name"];
		}
		else{
			
			$this->content_db->set_model(1);
			
		}
		//var_dump($info);
		//exit;
			$this->content_db->add_content($info);
			
		
		showM('添加成功',APP_PATH.'index.php?m=member&c=content&a=published');
