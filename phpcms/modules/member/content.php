...
class content extends foreground {
	...
	public function publish() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			include template('member', $template,$default_style);
	...
	public function published() {
		...
		$sitelist  = getcache('sitelist','commons');
			...
			include template('member','content_published',$default_style);
	}
	/**
	 * 编辑内容
	 */
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(isset($_GET['catid']) && $_GET['catid']) {
				...
				if($category['type']==0) {
					...
					include template('member','content_publish',$default_style);
	...
	public function info_publish() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			*/
			...
			include template('member','info_content_publish',$default_style);
		}
	}
	function info_top() {
		$exist_posids = array();
		...
		$sitelist  = getcache('sitelist','commons');
			...
			include template('member','info_top',$default_style);
	}
	...
	public function upload_video() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			$flash_info = $ku6api->param(); //加载视频上传工具信息
			...
			include template('member','upload_video',$default_style);
		}
	}
}
?>