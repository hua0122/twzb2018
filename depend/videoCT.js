...
$.fn.videoCt = function (options) {
    ...
    let _this = $(this).parents(".videoBox");
    // 控件结构
    var $videoCt = $(this);
    ...
    _this.find('.video-seek video').attr('src', opts.clarity.src[0]);
    ...
    var getVideoInf = function (){
        var setTimer;
        var key = true;
        if (( (new Date().getTime() - now_time)<3000 ) && ($videoCt[0].readyState != 4 && _this.find('.video-seek video')[0].readyState != 4)) {
            _this.find('.video-prompt').fadeOut();
            ...
            move(_this.find('.seek-slider'), $videoCt_seek, _this.find('.seek-con'), 8, videoJump, _this.find('.realTime'), true);
            _this.find('.seek-slider').css('left', intOpts.currentTime / intOpts.duration * _this.find('.video-seek').width() - 8);
            _this.find('.seek-con').css('width', intOpts.currentTime / intOpts.duration * _this.find('.video-seek').width());
            _this.find('.video-seek').hover(function () {
                key = true;
                ...
                key = false;
                clearTimeout(setTimer);
                _this.find('.video-seek .viewBox').fadeOut();
            });
            ...
            move(_this.find('.audio-slider'), _this.find('.audio-box'), _this.find('.audio-con'), 6, audioJump);
            _this.find('.audio-slider').css('left', intOpts.volume * _this.find('.audio-box').width() - 6);
            _this.find('.audio-con').css('width', intOpts.volume * _this.find('.audio-box').width());
            ...
            //弹幕
            // _this.find('.video-comment input[type=text]').off('focus').on('focus', function () {
            //     _this.find('.video-comment').addClass('commentActive');
            // });
            // _this.find('.video-comment p').off('click').on('click', function () {
            //     _this.find('.video-comment .setBox').fadeToggle();
            // });
            // _this.find('.video-comment .close').off('click').on('click', function () {
            //     $('.video-comment .setBox').fadeOut();
            // });
            // _this.find('.video-comment input[type=color]').on('input', function () {
            //     var wordColor = $(this).val();
            //     _this.find('.video-comment input[type=text]').css('color', wordColor);
            // });

            //发送弹幕
            // _this.find('.video-comment span').off('click').on('click', function () {
            //     var $that = _this.find('.video-comment');
            ...
            //     _this.find('.screenShootBox').append($('<p name='+ name +' class="screenShoot" data-status="0" data-action='+ action +' data-speed='+ speed +' style="color: '+ color +';top: '+ top +'%;font-size: '+ fontSize +'px">'+ word +'</p>'));
            //     _this.find('.video-comment input[type=text]').val("");
            // });

            //弹幕开关
            // _this.find('.video-barrage').off('click').on('click',function () {
            //     if(intOpts.commentSwitch){
            //         intOpts.commentSwitch = false;
            //         _this.find('.screenShootBox').fadeOut();
            //         $(this).addClass('barrageClose');
            //         _this.find('.video-comment').fadeOut();
            //     }else{
            //         intOpts.commentSwitch = true;
            //         _this.find('.screenShootBox').fadeIn();
            //         $(this).removeClass('barrageClose');
            //         _this.find('.video-comment').fadeIn();
            //     }
            // });
            ...
            //播放速度
            _this.find('.video-playSpeed').off('click').on('click',function () {
                //comStop();
            ...
            //     _this.find('.video-seek video').attr('src', intOpts.claritySrc);
            //     opts.autoplay = true;
            ...
            //下载
            // _this.find('.video-update').attr('download',intOpts.title);
            // _this.find('.video-update').attr('href',intOpts.claritySrc);
                ...
                if(!intOpts.status) {
                    ...
                    seTimer = setInterval(function () {
                        ...
                        if( vprpo < 1){
                            ...
                            _this.find('.seek-buffer').css('width',bL);
                            intStatus();
    ...
    function hoverPlay(e) {
        var view = e.pageX - _this.find('.video-seek').offset().left + 1;
        var time = $videoCt[0].duration * view / _this.find('.video-seek').width();
        if(view <= 30){
            _this.find('.video-seek .viewBox').css('left',-20);
            _this.find('.video-seek .viewBox i').css('left',view+15);
        }else if(view >= (_this.find('.video-seek').width()-30)){
            _this.find('.video-seek .viewBox').css('left',_this.find('.video-seek').width()-80);
            _this.find('.video-seek .viewBox i').css('left',view-_this.find('.video-seek').width()+75);
        }else{
            _this.find('.video-seek .viewBox').css('left',view - 50);
            _this.find('.video-seek .viewBox i').css('left',44);
        }
        _this.find('.video-seek video')[0].currentTime = time;
        _this.find('.video-seek span').text(gTimeFormat(time));
        _this.find('.video-seek .viewBox').fadeIn();
    }
    ...
    function intStatus(){
        if($videoCt[0].readyState == 4){
            _this.find('.video-status').fadeOut();
        }else{
            _this.find('.video-status').fadeIn();
            //comStop();
        }
    }
    ...
    //     if(comTimer != time){
    //         _this.find(".screenShoot[name="+ time +"][data-status='0']").each(function () {
    //             var that = $(this);
    //             var i = that.index();
    ...
    //         });
    //         _this.find(".screenShoot[data-status='2']").each(function () {
    //             var that = $(this);
    //             var i = that.index();
    ...
    //             that.attr('data-status',1);
    //         });
    //         _this.find(".screenShoot[data-status='3']").each(function () {
    ...
    //弹幕停止
    // function comStop() {
    //     _this.find(".screenShoot[data-status='1']").each(function () {
    //         var that = $(this);
    //         var i = that.index();
    ...
    //         _this.find('.screenShoot').each(function () {
    //             var data = {
    //                 "word": $(this).text(),
    ...
    function videoDom( opts ) {
        ...
        if( opts.barrage ){
            barrage.appendTo(_this.find(".video-controls"));
            _this.find(".video-controls").after($('<div class="screenShootBox"></div>'));
        }
        if( opts.reversal ){reversal.appendTo(_this.find(".video-controls"))}
        if( opts.playSpeed ){playSpeed.appendTo(_this.find(".video-controls"))}
        if( opts.update ){update.appendTo(_this.find(".video-controls"))}
        if( opts.clarity.type && opts.clarity.src ){
            ...
            $(clarity).appendTo(_this.find(".video-controls"));
        }
        if( opts.comment ){comment.appendTo(_this.find(".video-controls"))}
    }
